...
We expect the distribution of beta values to be a little more extreme:
...
normexped = normexp(TMM[1000:2000,], offset=50)
par(mfrow=c(1,2))
hist(betas(TMM)[1000:2000,], main='before normexp')
hist(betas(normexped), main='after normexp')
@ 
...
gammactls = gamma.ctl(TMM[1000:2000,], offset=50)
par(mfrow=c(1,2))
hist(betas(TMM)[1000:2000,], main='before gammactl')
hist(betas(gammactls), main='after gammactl')
@ 
...
## library(rGammaGamma)
source("~/Dropbox/rGammaGamma/pkg/R/gammamix.R")
...
corrected = gamma.mix(TMM[1000:2000,], offset=50)
par(mfrow=c(1,2))
hist(betas(TMM)[1000:2000,], main='before gammamix')
hist(betas(corrected), main='after gammamix')
@ 
